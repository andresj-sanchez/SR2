...
Usage:
  python tools/decomp-context.py -u Develop/Projects/SR2/pgm/src/Object/Player/Performance.cpp -f "clsPrfm::setup"
  python tools/decomp-context.py -u Develop/Projects/SR2/pgm/src/Object/Player/Performance.cpp -f "clsPrfm::setup" --no-ghidra
  python tools/decomp-context.py -u Develop/Projects/SR2/pgm/src/Object/Player/Performance.cpp -f "clsPrfm::setup" --ghidra-version gc
  python tools/decomp-context.py -u Develop/Projects/SR2/pgm/src/Object/Player/Performance.cpp -f "clsPrfm::setup" --lookup-mode signature
  python tools/decomp-context.py -u Develop/Projects/SR2/pgm/src/Object/Player/Performance.cpp -f "clsPrfm::setup" --no-lookup
  python tools/decomp-context.py -u Develop/Projects/SR2/pgm/src/Object/Player/Performance.cpp -f "clsPrfm::setup" --no-source
...
_SYMBOLS_FILE = os.path.join(root_dir, "config", "SLUS-21642-PROTO-070901", "symbol_addrs.txt")
GC_SYMBOLS_FILE = _SYMBOLS_FILE
PS2_SYMBOLS_FILE = _SYMBOLS_FILE
GC_DWARF_PATH = os.path.join(root_dir, "symbols", "Dwarf")
DEBUG_LINES_FILE = os.path.join(root_dir, "symbols", "debug_lines.txt")
GC_GHIDRA_PROGRAM = "main.dol"
PS2_GHIDRA_PROGRAM = "SLUS_216.42"
...
def find_symbol_in_diff(
    ...
    stripped_pattern = strip_template_args(func_name).lower()

    def matches_symbol(sym: Dict[str, Any]) -> bool:
        name = sym.get("demangled_name", sym.get("name", ""))
        mangled = sym.get("name", "")
        if pattern in name.lower() or pattern in mangled.lower():
            return True
        stripped_name = strip_template_args(name).lower()
        stripped_mangled = strip_template_args(mangled).lower()
        return stripped_pattern in stripped_name or stripped_pattern in stripped_mangled

    for sym in left_syms:
        if matches_symbol(sym):
            right_sym = None
...
def check_ghidra() -> None:
    ...
    try:
        ...
        for prog in [GC_GHIDRA_PROGRAM, PS2_GHIDRA_PROGRAM]:
            if prog in output:
                ...
                print(f"      Run: ghidra set-default project SR2")
    ...
    # Quick decompile smoke test — test PS2 (primary) and Wii (future) programs separately
    symbols_file = PS2_SYMBOLS_FILE if os.path.exists(PS2_SYMBOLS_FILE) else GC_SYMBOLS_FILE
        ...
        with open(symbols_file) as f:
            for line in f:
                ...
                if m:
                    ...
                    break
        if test_addr:
            # PS2 is the primary target — failures here are hard FAILs
            code, err = ghidra_decompile(test_addr, PS2_GHIDRA_PROGRAM)
            if code is not None:
                print(f"OK    PS2 decompile smoke test passed (0x{test_addr})")
            else:
                print(f"FAIL  PS2 decompile smoke test failed for 0x{test_addr}: {err}")
            # Wii is a future target — failures are WARNs until PowerPC language is installed
            code, err = ghidra_decompile(test_addr, GC_GHIDRA_PROGRAM)
            if code is not None:
                print(f"OK    Wii decompile smoke test passed (0x{test_addr})")
            else:
                print(f"WARN  Wii decompile smoke test failed for 0x{test_addr}: {err}")
    else:
        print(f"SKIP  smoke test — symbols file not found: {symbols_file}")
...
def resolve_repo_path_from_debug_path(debug_path: str) -> Optional[str]:
    ...
    suffixes: List[str] = []
    if "/develop/projects/sr2/" in lowered:
        suffixes.append(lowered.split("/develop/projects/sr2/", 1)[1].lstrip("/"))
    if "/src/" in lowered:
    ...
    matches: List[str] = []
    for search_root in [os.path.join(root_dir, "src"), os.path.join(root_dir, "include")]:
...
def tu_name_from_unit(unit: Dict[str, Any]) -> str:
    ...
    if source:
        # src/Develop/Projects/SR2/pgm/src/Object/Player/Performance.cpp -> Performance.o
...
def main():
    ...
    parser.add_argument(
        "-u", "--unit", help="Unit name (e.g. Develop/Projects/SR2/pgm/src/Object/Player/Performance.cpp)"
    )
    ...
    parser.add_argument(
        ...
        choices=["both", "gc", "ps2"],
        default="ps2",
        help="Choose which Ghidra decompile(s) to show (default: ps2)",
    )
    parser.add_argument(
    ...
    if not args.no_ghidra and (left_sym or right_sym):
        ...
        if args.ghidra_version in ("both", "gc"):
            print_ghidra_decompilation(
                "RS9E8P", GC_SYMBOLS_FILE, GC_GHIDRA_PROGRAM, args.function, mangled
            )
