...
DEFAULT_ELF_BY_GAME = {
    "ps2": os.path.join(ROOT_DIR, "orig", "SLUS-21642-PROTO-070901", "SLUS_216.42"),
}
...
def find_section_for_address(elffile: ELFFile, address: int) -> Optional[Dict[str, Any]]:
    ...
    for section in elffile.iter_sections():
        ...
        if not (int(header["sh_flags"]) & SHF_ALLOC):
            continue  # non-loadable section (debug, strtab, symtab, etc.)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="ps2",
        help="Shortcut for selecting the default ELF (default: ps2)",
    )
    parser.add_argument(
        "--elf",
