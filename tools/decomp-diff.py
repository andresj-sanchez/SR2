...
def build_diff(data: Dict[str, Any], symbol_name: str, args) -> None:
    ...
    if args.unified:
        ...
        if args.full:
            # No hunk windowing — show every instruction
            for a, m, lt, rt, _, _, ls in expanded:
        ...
        # (same algorithm as real diff -u: if gap between two mismatch regions
        # is <= 2*context, merge them so context lines aren't printed twice)
        mismatch_ranges = []
        i = 0
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Show every instruction without collapsing or hunk windowing. "
            "In side-by-side mode this equals --no-collapse. "
            "In --unified mode all instructions are shown, not just changed hunks."
        ),
    )
    ...
    # --full implies --no-collapse for the side-by-side path
    if args.full:
