...
def main() -> None:
    ...
    if not any(os.path.isdir(d) for d in SEARCH_DIRS):
        print(f"Error: no search directories found: {SEARCH_DIRS}", file=sys.stderr)
        sys.exit(1)

    matches = search_tree(args.name, kinds)

    if not matches:
        searched = ", ".join(os.path.relpath(d, root_dir) for d in SEARCH_DIRS if os.path.isdir(d))
        print(f"Not found: '{args.name}' in {searched}")
