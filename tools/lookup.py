...
re_struct_decl_name = re.compile(r"^(?:struct|class)\s+([A-Za-z_]\w*)\b")
...
re_global_name = re.compile(r"^(?:[\w\s:<>*&]+?)\b(\w+)(?:\s*\[\d*\])*\s*;")
re_global_funcptr_name = re.compile(r"\(\*\s*(\w+)(?:\s*\[\d*\])*\s*\)")
re_symbol_addr = re.compile(
    r"^\s*(?P<name>\S+)\s*=\s*(?P<addr>0x[0-9A-Fa-f]+);\s*"
    r"//\s*size:(?P<size>[0-9A-Fa-fx]+)(?P<meta>.*)$"
)
    ...
    for line in lines:
        if not in_block:
            ...
            if pending:
                ...
                if "{" in line and re_struct_decl_name.match(line):
                    pending.append(line)
                    current = pending
                    current_decl_line = line
                    in_block = True
                    brace_depth = sum(ln.count("{") - ln.count("}") for ln in current)
                    pending = []
                    # Single-line type (e.g. class Foo {};): already complete
                    if brace_depth == 0 and current[-1].rstrip().endswith(";"):
                        block = "".join(current)
                        m = re_struct_decl_name.match(current[-1])
                        if m:
                            results.append((m.group(1), block))
                        in_block = False
                else:
                    # Not a named struct/class declaration, discard pending. This
                    # skips anonymous one-line records such as vtable placeholders.
                    pending = []
            continue
        ...
        if brace_depth == 0 and line.rstrip().endswith("};"):
            block = "".join(current)
            m = re_struct_decl_name.match(current_decl_line)
            if m:
                results.append((m.group(1), block))
            in_block = False
...
def find_function_by_address(
    ...
    q = int(query, 16)
    # Two-pass: prefer a start-address match so that when address X is
    # simultaneously the END of function A and the START of function B,
    # we return B (the function defined at that address), not A.
...
def find_symbol_addr_functions_by_address(
    symbol_addrs_path: str, query: str
) -> list[str]:
    """Return symbol_addrs.txt function entries matching or containing *query*.

    lookup.py's primary source for functions is DWARF. Some weak/duplicated
    functions have symbol entries but no function body in functions.nothpp; this
    fallback makes that distinction visible instead of reporting a false miss.
    """
    ...
    with open(symbol_addrs_path, "r", encoding="utf-8", errors="ignore") as f:
        for raw_line in f:
            line = raw_line.rstrip()
            match = re_symbol_addr.match(line)
            if not match:
                continue
            ...
            size = (
                int(size_text, 16)
                if size_text.lower().startswith("0x")
                else int(size_text)
            )
            if size <= 0:
                continue
            ...
            elif start < q < start + size:
                containing_matches.append(line)

    return exact_matches or containing_matches


def default_symbol_addrs_path() -> str:
    repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    return os.path.join(
        repo_root, "config", "SLUS-21642-PROTO-070901", "symbol_addrs.txt"
    )
...
def split_line_decls(text: str, name_re: re.Pattern) -> list[tuple[str, str]]:
    ...
    for line in lines:
        ...
        m = re_global_funcptr_name.search(code) if name_re is re_global_name else None
        if not m:
...
def main():
    ...
    if args.file:
        ...
        if not args.source:
            # Default to symbols/Dwarf — the standard GC_DWARF folder for this project.
    ...
    if kind == "struct":
        ...
        if _looks_like_address(query):
            ...
            if result is None:
                symbol_matches = find_symbol_addr_functions_by_address(
                    default_symbol_addrs_path(), query
                )
                if symbol_matches:
                    print(
                        "// Not present in symbols/Dwarf/functions.nothpp; "
                        "exact or containing symbol_addrs.txt match:"
                    )
                    print("\n".join(symbol_matches))
                    return
