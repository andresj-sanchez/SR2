...
During edit loops, use targeted validation before Ninja builds:

```sh
python tools/decomp-workflow.py validate Develop/Projects/SR2/pgm/src/Object/Player/Performance.cpp
```

This runs direct guards/checks for the chosen file and direct counterpart without invoking Ninja. Use it especially after guard/check tool edits, because those edits invalidate many Ninja guard stamps.
...
Determine the file path (e.g. `src/Develop/Projects/SR2/pgm/src/Object/Player/Performance.cpp`).
SR2 does **not** use unity/jumbo builds — each `.cpp` is compiled directly.
...
python tools/decomp-workflow.py unit -u Develop/Projects/SR2/pgm/src/Object/Player/Performance.cpp --limit 20
```
...
starting point because it favors large remaining gains and penalizes near-finished
...
If the TU depends on a type whose definition does not yet exist in the project, follow
the scaffold workflow in `.github/skills/scaffold/SKILL.md` to create the needed
header/source definitions before moving on.
...
- **Generalizable patterns** (e.g. MIPS instruction scheduling quirks, MWCC-specific
  optimizations) should be noted for future sessions.
- **TU-specific patterns** should be kept in the session context and applied to
...
python tools/decomp-workflow.py diff -u Develop/Projects/SR2/pgm/src/Object/Player/Performance.cpp -s nonmatching -t function
